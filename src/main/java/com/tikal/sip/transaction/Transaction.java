...
public abstract class Transaction implements MediaEventListener<SdpPortManagerEvent>  {
	...
	protected Transaction (String method, SipEndPointImpl localParty, Address remoteParty) {
		this.method= method;
	...
	protected void processSdpOffer(byte[] rawSdp) throws ServerInternalErrorException {
	...
	protected void processSdpAnswer (byte[] rawSdp) throws ServerInternalErrorException {
	...
	public javax.media.mscontrol.networkconnection.NetworkConnection getNetworkConnection () {
		return networkConnection;
	}
	
	public void processTimeOut(TimeoutEvent timeoutEvent) {
		// This function avoids every transaction to define processTimeOut
		log.error("Time Out while waiting a response from client transaction: " + method);
		release();
	}
